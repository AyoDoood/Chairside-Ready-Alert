...
cd "$(dirname "$0")"
SCRIPT_DIR="$(pwd -P)"
SOURCE_PY="$SCRIPT_DIR/dental_messenger.py"
INSTALL_DIR="$HOME/Library/Application Support/DentalMessenger"
DESKTOP_APP="$HOME/Desktop/Chairside Messenger.app"
...
CACHE_DIR="$HOME/Library/Caches/DentalMessenger"

echo "[Chairside Messenger] Installer"
echo ""

if [[ ! -f "$SOURCE_PY" ]]; then
  echo "Error: dental_messenger.py not found next to this installer."
  echo "Keep this .command file in the same folder as dental_messenger.py."
  read -r
  exit 1
fi
...
mkdir -p "$INSTALL_DIR"
cp -f "$SOURCE_PY" "$INSTALL_DIR/dental_messenger.py"
...
rm -f "$HOME/Desktop/Dental Messenger.command"
rm -rf "$HOME/Desktop/Dental Messenger Shortcut.app"
rm -rf "$DESKTOP_APP"
...
# framework lib on DYLD path when not started from a login shell.
{
  ...
  echo '{ date; echo "Chairside Messenger — errors: shortcut_stderr.txt and startup_log.txt"; } >>"$LOG" 2>/dev/null || true'
  printf 'exec %q -u dental_messenger.py 2>>"$LOG"\n' "$LAUNCH_BIN"
} > "$APP_MACOS/launcher"
...

{
  ...
  echo '  <string>Chairside Messenger</string>'
...
echo "Desktop shortcut: Chairside Messenger.app"
echo ""
echo "Launching Chairside Messenger..."
open "$DESKTOP_APP"
echo "Done."
